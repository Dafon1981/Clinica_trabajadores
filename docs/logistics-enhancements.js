...
window.registerLogisticsFixes(({ state, $, esc, fmt, fmtDate, toast, rpc, sessionToken, clearSession, navAdmin, navWorker, goPage, adminProducts, loadOrders, renderOrderList, panel, bindJumps }) => {
  ...
  const renderInventory = async function () {
    ...
    $('#page-content').innerHTML = panel(`<div class="panel-header"><div><h2>Inventario general</h2><p class="muted">Busca por código, registra el conteo físico y guarda el inventario. Solo los productos con conteo escrito se ajustan en Stock y Kardex.</p></div><div class="stock-actions no-print"><button id="inventory-refresh" class="button outline" type="button">Actualizar montos</button><button id="inventory-pdf" class="button ghost" type="button">PDF del conteo actual</button><button id="inventory-last-pdf" class="button ghost" type="button">PDF último inventario de hoy</button><button id="inventory-close" class="button primary" type="button">Guardar y actualizar stock</button></div></div><div class="grid cards inventory-summary" id="inventory-summary"></div><div class="toolbar no-print"><input id="inventory-search" class="input" placeholder="Buscar código o descripción"><label class="field-label inventory-observation">Observación<textarea id="inventory-observation" rows="2" placeholder="Ej. Inventario general de almacén, turno mañana"></textarea></label></div><div class="table-wrap"><table class="table"><thead><tr><th>Código</th><th>Descripción</th><th>Unidad</th><th>Stock sistema</th><th>Conteo físico</th><th>Diferencia</th></tr></thead><tbody id="inventory-body"></tbody></table></div>`);
    const today = new Date().toISOString().slice(0, 10);
    state.inventoryRangeReports = state.inventoryRangeReports || [];
    state.inventoryReportSelection = state.inventoryReportSelection || {};
    $('#page-content').insertAdjacentHTML('beforeend', panel(`<section class="inventory-report-panel"><div class="panel-header"><div><h2>Reportes de inventario</h2><p class="muted">Elige un rango de fechas y une uno o varios inventarios cerrados en un solo PDF para el encargado de pedidos.</p></div><div class="stock-actions no-print"><button id="inventory-report-download" class="button ghost" type="button" disabled>Descargar PDF</button><button id="inventory-report-share" class="button primary" type="button" disabled>Compartir PDF</button></div></div><div class="toolbar no-print"><label class="field-label">Desde<input id="inventory-report-from" class="input" type="date" value="${today}"></label><label class="field-label">Hasta<input id="inventory-report-to" class="input" type="date" value="${today}"></label><button id="inventory-report-load" class="button outline" type="button">Buscar inventarios</button></div><div id="inventory-report-summary" class="muted">Selecciona un rango y pulsa Buscar inventarios.</div><div class="table-wrap"><table class="table"><thead><tr><th>Unir</th><th>Fecha y hora</th><th>Código</th><th>Responsable</th><th>Contados</th><th>Ajustes</th><th>Observación</th></tr></thead><tbody id="inventory-report-body"><tr><td colspan="7" class="empty-state">Aún no hay reporte cargado.</td></tr></tbody></table></div></section>`));
    ...
    const hasSameStock = (stockSystem, stockCounted) => Math.abs(Number(stockSystem) - Number(stockCounted)) < 0.000001;
    const updateSummary = () => {
      const lines = enteredLines();
      const matching = lines.filter((line) => hasSameStock(products.find((row) => row.codigo === line.codigo)?.stock_actual, line.stock_contado)).length;
      const adjusted = lines.length - matching;
      $('#inventory-summary').innerHTML = `<article class="metric"><p>Productos activos</p><b>${products.length}</b></article><article class="metric"><p>Contados</p><b>${lines.length}</b></article><article class="metric inventory-match"><p>Coinciden con stock</p><b>${matching}</b><small>Conteo igual al sistema</small></article><article class="metric"><p>Con diferencia</p><b>${adjusted}</b><small>Requieren revisión</small></article><article class="metric"><p>Estado</p><b>${state.lastAdminInventory?.codigo_inventario || 'En conteo'}</b></article>`;
    };
    const draw = (filter = '') => {
      ...
      $('#inventory-body').innerHTML = visible.map((product) => {
        ...
        const rowClass = difference === null ? '' : hasSameStock(product.stock_actual, counted) ? 'inventory-match' : difference < 0 ? 'critical-row' : 'inventory-positive';
        const differenceLabel = difference === null ? '—' : hasSameStock(product.stock_actual, counted) ? '<span class="badge done">COINCIDE</span>' : `<b>${difference > 0 ? '+' : ''}${fmt(difference)}</b>`;
        return `<tr class="${rowClass}"><td><b>${esc(product.codigo)}</b></td><td>${esc(product.descripcion)}</td><td>${esc(product.unidad || '')}</td><td>${fmt(product.stock_actual)}</td><td><input class="inventory-count input" data-inventory-count="${esc(product.codigo)}" type="number" min="0" step="0.001" inputmode="decimal" value="${esc(raw)}" placeholder="—"></td><td>${differenceLabel}</td></tr>`;
    ...
    const exportLastDailyPdf = async () => {
      const button = $('#inventory-last-pdf'); button.disabled = true;
      try {
        const result = await rpc('log_ultimo_inventario_diario', { p_token: sessionToken() });
        const inventory = result?.[0]; const rows = inventory?.items || [];
        if (!inventory || !rows.length) return toast('Aún no hay un inventario cerrado hoy para descargar.', true);
        const { jsPDF } = window.jspdf; const doc = new jsPDF({ unit: 'mm', format: 'a4', orientation: 'landscape' });
        doc.setFontSize(14); doc.text('ÚLTIMO INVENTARIO CERRADO DEL DÍA', 14, 15); doc.setFontSize(8);
        doc.text(`Centro de Diálisis Virgen del Lourdes · Código: ${inventory.codigo_inventario}`, 14, 21);
        doc.text(`Cerrado: ${fmtDate(inventory.cerrado_en)} · Responsable: ${inventory.responsable}`, 14, 26);
        doc.text(`Productos contados: ${inventory.productos_contados} · Ajustes de Kardex: ${inventory.productos_ajustados}`, 14, 31);
        doc.autoTable({ startY: 36, head: [['Código', 'Descripción', 'UM', 'Stock sistema', 'Conteo físico', 'Diferencia']], body: rows.map((row) => [row.codigo, row.descripcion, row.unidad || '', fmt(row.stock_sistema), fmt(row.stock_contado), `${Number(row.diferencia) > 0 ? '+' : ''}${fmt(row.diferencia)}`]), styles: { fontSize: 7 }, headStyles: { fillColor: [17, 132, 117] } });
        const finalY = doc.lastAutoTable.finalY + 15; doc.setFontSize(8); doc.text(`Observación: ${inventory.observacion || '—'}`, 14, finalY); doc.text('RESPONSABLE DE INVENTARIO', 28, finalY + 26); doc.text('REVISIÓN DE ADMINISTRACIÓN', 138, finalY + 26); doc.line(14, finalY + 21, 95, finalY + 21); doc.line(120, finalY + 21, 205, finalY + 21);
        await shareFile(doc.output('blob'), `Ultimo_inventario_${String(inventory.codigo_inventario).replace(/[^A-Za-z0-9_-]/g, '_')}.pdf`, 'Último inventario del día');
        toast('PDF del último inventario diario preparado.');
      } catch (error) { toast(error.message || 'No se pudo generar el PDF del último inventario.', true); }
      finally { if ($('#inventory-last-pdf')) $('#inventory-last-pdf').disabled = false; }
    };
    const reportItems = (report) => {
      if (Array.isArray(report.items)) return report.items;
      try { return JSON.parse(report.items || '[]'); } catch { return []; }
    };
    const selectedReports = () => (state.inventoryRangeReports || []).filter((report) => state.inventoryReportSelection[String(report.inventario_id)]);
    const renderRangeReports = () => {
      const reports = state.inventoryRangeReports || []; const selected = selectedReports();
      $('#inventory-report-summary').textContent = reports.length ? `${reports.length} inventario(s) encontrado(s). ${selected.length} seleccionado(s) para un PDF consolidado.` : 'No hay inventarios cerrados en el rango elegido.';
      $('#inventory-report-body').innerHTML = reports.map((report) => `<tr><td><input type="checkbox" data-inventory-report-select="${report.inventario_id}" ${state.inventoryReportSelection[String(report.inventario_id)] ? 'checked' : ''}></td><td>${fmtDate(report.cerrado_en)}</td><td><b>${esc(report.codigo_inventario)}</b></td><td>${esc(report.responsable || 'Administración')}</td><td>${report.productos_contados}</td><td>${report.productos_ajustados}</td><td>${esc(report.observacion || '—')}</td></tr>`).join('') || '<tr><td colspan="7" class="empty-state">No hay inventarios cerrados en este rango.</td></tr>';
      $('#inventory-report-download').disabled = !selected.length; $('#inventory-report-share').disabled = !selected.length;
      document.querySelectorAll('[data-inventory-report-select]').forEach((input) => input.onchange = () => { state.inventoryReportSelection[input.dataset.inventoryReportSelect] = input.checked; renderRangeReports(); });
    };
    const buildRangeReportPdf = (reports) => {
      const { jsPDF } = window.jspdf; const doc = new jsPDF({ unit: 'mm', format: 'a4', orientation: 'landscape' });
      const rows = reports.flatMap((report) => reportItems(report).map((item) => {
        const stock = Number(item.stock_sistema); const counted = Number(item.stock_contado);
        const critical = stock > 0 && counted <= stock * 0.2;
        return { ...item, fecha: fmtDate(report.cerrado_en), inventario: report.codigo_inventario, responsable: report.responsable || 'Administración', critical };
      })).sort((a, b) => Number(b.critical) - Number(a.critical) || String(a.codigo).localeCompare(String(b.codigo)));
      const criticalCount = rows.filter((row) => row.critical).length;
      doc.setFontSize(14); doc.text('REPORTE CONSOLIDADO DE INVENTARIO', 14, 15); doc.setFontSize(8);
      doc.text(`Centro de Diálisis Virgen del Lourdes · Inventarios unidos: ${reports.length} · Productos reportados: ${rows.length}`, 14, 21);
      doc.text(`Alertas críticas: ${criticalCount} (conteo físico igual o menor al 20% del stock del sistema)`, 14, 26);
      doc.autoTable({ startY: 31, head: [['Fecha', 'Inventario', 'Código', 'Descripción', 'UM', 'Stock sistema', 'Conteo físico', 'Diferencia', 'Alerta']], body: rows.map((row) => [row.fecha, row.inventario, row.codigo, row.descripcion, row.unidad || '', fmt(row.stock_sistema), fmt(row.stock_contado), `${Number(row.diferencia) > 0 ? '+' : ''}${fmt(row.diferencia)}`, row.critical ? 'CRÍTICO ≤20%' : 'Normal']), styles: { fontSize: 6.5 }, headStyles: { fillColor: [17, 132, 117] }, didParseCell: (data) => { if (data.section === 'body' && rows[data.row.index]?.critical) { data.cell.styles.fillColor = [254, 226, 226]; data.cell.styles.textColor = [153, 27, 27]; data.cell.styles.fontStyle = 'bold'; } } });
      const finalY = doc.lastAutoTable.finalY + 15; doc.setFontSize(8); doc.text('Las filas rojas requieren revisión y posible pedido de reposición.', 14, finalY); doc.text('RESPONSABLE DE INVENTARIO', 28, finalY + 26); doc.text('REVISIÓN DE ADMINISTRACIÓN', 138, finalY + 26); doc.line(14, finalY + 21, 95, finalY + 21); doc.line(120, finalY + 21, 205, finalY + 21);
      return doc;
    };
    ...
    $('#inventory-refresh').onclick = async () => {
      const button = $('#inventory-refresh'); button.disabled = true;
      try { await renderInventory(); toast('Montos del sistema actualizados. Tu conteo físico se conserva.'); }
      catch (error) { button.disabled = false; toast(error.message || 'No se pudieron actualizar los montos.', true); }
    };
    $('#inventory-pdf').onclick = () => { void exportPdf(); };
    $('#inventory-last-pdf').onclick = () => { void exportLastDailyPdf(); };
    $('#inventory-report-load').onclick = async () => {
      const desde = $('#inventory-report-from').value; const hasta = $('#inventory-report-to').value;
      if (!desde || !hasta) return toast('Elige la fecha inicial y final.', true);
      const button = $('#inventory-report-load'); button.disabled = true;
      try { const reports = await rpc('log_listar_inventarios_rango', { p_token: sessionToken(), p_desde: desde, p_hasta: hasta }); state.inventoryRangeReports = reports || []; state.inventoryReportSelection = Object.fromEntries((reports || []).map((report) => [String(report.inventario_id), true])); renderRangeReports(); }
      catch (error) { toast(error.message || 'No se pudieron cargar los inventarios.', true); }
      finally { if ($('#inventory-report-load')) $('#inventory-report-load').disabled = false; }
    };
    $('#inventory-report-download').onclick = () => { const reports = selectedReports(); if (!reports.length) return; buildRangeReportPdf(reports).save(`Reportes_inventario_${$('#inventory-report-from').value}_a_${$('#inventory-report-to').value}.pdf`); };
    $('#inventory-report-share').onclick = async () => { const reports = selectedReports(); if (!reports.length) return; const doc = buildRangeReportPdf(reports); await shareFile(doc.output('blob'), `Reportes_inventario_${$('#inventory-report-from').value}_a_${$('#inventory-report-to').value}.pdf`, 'Reportes de inventario'); };

  ...
  const renderInventory = async function () {
    ...
    const buildRangeReportPdf = (reports) => {
      ...
      reports.forEach((report, index) => {
        if (index) doc.addPage(); const rows = reportItems(report);
        doc.setFontSize(14); doc.text('REPORTE DE INVENTARIO CERRADO', 14, 15); doc.setFontSize(8);
        doc.text(`Centro de Diálisis Virgen del Lourdes · Código: ${report.codigo_inventario}`, 14, 21);
        doc.text(`Cerrado: ${fmtDate(report.cerrado_en)} · Responsable: ${report.responsable || 'Administración'}`, 14, 26);
        doc.text(`Productos contados: ${report.productos_contados} · Ajustes de Kardex: ${report.productos_ajustados}`, 14, 31);
        doc.autoTable({ startY: 36, head: [['Código', 'Descripción', 'UM', 'Stock sistema', 'Conteo físico', 'Diferencia']], body: rows.map((row) => [row.codigo, row.descripcion, row.unidad || '', fmt(row.stock_sistema), fmt(row.stock_contado), `${Number(row.diferencia) > 0 ? '+' : ''}${fmt(row.diferencia)}`]), styles: { fontSize: 7 }, headStyles: { fillColor: [17, 132, 117] } });
        const finalY = doc.lastAutoTable.finalY + 15; doc.setFontSize(8); doc.text(`Observación: ${report.observacion || '—'}`, 14, finalY); doc.text('RESPONSABLE DE INVENTARIO', 28, finalY + 26); doc.text('REVISIÓN DE ADMINISTRACIÓN', 138, finalY + 26); doc.line(14, finalY + 21, 95, finalY + 21); doc.line(120, finalY + 21, 205, finalY + 21);
      });
      return doc;
    };
